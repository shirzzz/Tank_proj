...
bool TankAlgorithm::isDangerAhead() {
    ...
    std::pair<int, int> dir = directionToVector(canon_direction);  // Itai look at this line I used canon_direction instead of my_tank_ref.getCanonDirection()
    int dx = dir.first;
    int dy = dir.second;
    int fx = my_tank.first + dx;  // FIXED: Use my_tank_ref instead of tank
    int fy = my_tank.second+ dy;  // FIXED: Use my_tank_ref instead of tank
...
int TankAlgorithm::isAlignedHorizontally() {
    ...
    for(std::pair<size_t, size_t> opponent : opponents) {
        ...
        if (my_tank.second == (opponent.second)) {  // //Itai I deleted the cast to int here because my_tank.second is already an int
            return i;
        }
        i++;
    }
...
int TankAlgorithm::isAlignedVertically() {
    ...
    for(std::pair<size_t, size_t> opponent : opponents) {
        ...
        if (my_tank.first == (opponent.first)) {  // Itai I deleted the cast to int here because my_tank.first is already an int
            return i;
        }
        i++;
    }
...
int TankAlgorithm::isFacingOpponent() {  // FIXED: Added missing return type 'int'
    ...
    for(std::pair<size_t, size_t> opponent : opponents) {
        ...
        int dx = static_cast<int>(opponent.first) - my_tank.first;   // FIXED: Cast size_t to int
        int dy = static_cast<int>(opponent.second) - my_tank.second; // FIXED: Cast size_t to int
        if (dx == 0 && dy == 0) {
            i++;
            continue; // Same position, check next opponent
        }
        CanonDirection dirToOpponent = getDirectionFromDelta(dx, dy);
        if (canon_direction == dirToOpponent) { //Itai here I used canon_direction instead of my_tank_ref.getCanonDirection()
